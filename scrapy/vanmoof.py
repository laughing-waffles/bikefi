...
class VanmoofSpider(scrapy.Spider):
    name = 'vanmoof'
    allowed_domains = ['vanmoof.com']
    start_urls = ['http://vanmoof.com/']

    rules = [Rule(LinkExtractor(allow='/'),
        callback='parse_filter_bike', follow=True)]

    def parse_filter_bike(self, response):
        exists = response.xpath('//a[@class="m-cta m-cta--rounded m-cta--full b-hero__link b-hero__link--dark m-cta--primary-dark"]').get()
        if exists:
            ...
            name = response.xpath('//h1[@class="b-hero__title"]/text()').get()
